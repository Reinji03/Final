...
st.caption("노마드 소비데이터를 활용하여 소비 패턴 분석")
...
with st.container():
    ...
    with col1:
        ...
        # Choropleth 지도 생성 (Mapbox 스타일)
        fig_map = px.choropleth(
            region_sales,
            geojson=geojson_data,
            featureidkey='properties.SGG_NM',  # GeoJSON에서 구 이름과 매칭
            locations='Region',
            color='Total Sales',
            color_continuous_scale="YlGnBu",  # 색상 팔레트 변경
        )
        ...
        fig_map.update_geos(
            visible=False,
            fitbounds="locations",
            projection_type="mercator",  # Mercator 투영법 사용
            showcoastlines=True,
            coastlinecolor="Black",  # 해안선 색상
            showland=True,
            landcolor="white",  # 땅 색상
            showlakes=True,
            lakecolor="lightblue",  # 호수 색상
        )

        # 투명도 설정
        fig_map.update_traces(marker=dict(opacity=0.6))

        # Mapbox 스타일 적용 (세련된 지도 스타일)
        fig_map.update_layout(
            mapbox_style="carto-positron",  # 세련된 스타일의 Mapbox 제공
            mapbox_accesstoken="YOUR_MAPBOX_ACCESS_TOKEN",  # Mapbox 토큰 입력
        )
